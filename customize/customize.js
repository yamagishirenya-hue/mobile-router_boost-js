(function() {
    ...
    // 桁数制限（maxlength）を強制適用する関数
    const applyMaxlength = () => {
        for (let id in limitConfig) {
            const el = document.querySelector(`[field-id="${id}"] input`);
            if (el) el.setAttribute("maxlength", limitConfig[id]);
        }
    };
    ...
    const timer = setInterval(() => {
        if (typeof kb !== 'undefined' && kb.event) {
            ...
            // 郵便番号入力時のハイフン除去（数値のみに制限）
            kb.event.on('kb.change.郵便番号', (ev) => {
                if (ev.record["郵便番号"].value) {
                    ev.record["郵便番号"].value = ev.record["郵便番号"].value.replace(/[^\d]/g, "");
                }
                return ev;
            });
